    ...
    async def get_bearer_token(self) -> str:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                raise HTTPException(status_code=500, detail="Failed to get bearer token")
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    print(f"SMS service error: {response.status_code} - {response.text}")
                    return False
...
class SMSVerificationService:
    ...
    async def get_active_session(self, phone_number: str) -> Optional[SMSVerificationSession]:
        stmt = select(SMSVerificationSession).where(
            and_(
                SMSVerificationSession.phone_number == phone_number,
                SMSVerificationSession.verified == False,
    ...
    async def store_verification_session(self, session_data: dict):
        # Invalidate any existing sessions for this phone number
    ...
    async def invalidate_existing_sessions(self, phone_number: str):
        ...
        for session in sessions:
            session.verified = True  # Mark as used
